...
const API_SERVICE = {
  ...
  async fetchLocationDetails(placeId: string) {
    try {
      const response = await fetch(
        `${this.BASE_URL}/place/details/json?place_id=${placeId}&key=AlzaSyyvH1149qgeoo1Xyvr6B3uhES5cYbQWfPv`
      );
  ...
  async fetchRouteInfo(origin: Location, destination: Location) {
    try {
      const response = await fetch(
        `${this.BASE_URL}/distancematrix/json?origins=${origin.name}&destinations=${destination.name}&key=AlzaSyyvH1149qgeoo1Xyvr6B3uhES5cYbQWfPv`
      );
      const data = await response.json();
    ...
    const fetchData = async () => {
      try {
        ...
        const [departureDetails, destinationDetails] = await Promise.all([
          API_SERVICE.fetchLocationDetails(departureId),
          API_SERVICE.fetchLocationDetails(destinationId)
        ]);
        ...
        const route = await API_SERVICE.fetchRouteInfo(
          { id: departureId },
          { id: destinationId }
        );
